...
def dorking(dork, fullurl):
    ...
    try:
        ...
        while page <= 500:
            ...
            if len(domain) != 0:
                print(f'{cyan}[OK] TOTAL DOMAIN >>{white} '+str(len(alldomain)))
                print(f'{yellow}[DORK]{white} '+str(dork))
                for doms in domain:
                    if (fullurl == 'y') or (fullurl == 'Y'):
                        doms = urllib.parse.unquote(doms)
                        if doms in alldomain:
                            print(f'{red}[DUPLICATE]{white} '+doms)
                        else:
                            print(f'{green}[*] '+doms+' [*]')
                        ...
                        if doms in alldomain:
                            ...
                            print('{red}[DUPLICATE]{white}{doms}')
                ...
                print(f'{red}[NOT OK] NO RESULTS FOUND!!!{white}')
...
"""
    try:
        print(ban)
        xxx = open(input('List dork ~# '), 'r').read().splitlines()
